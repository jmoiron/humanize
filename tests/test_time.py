...
@pytest.mark.parametrize(
    "arg, expected",
    [
        (NOW + dt.timedelta(seconds=ONE_MICROSECOND), "now"),
        (NOW + dt.timedelta(seconds=FOUR_MICROSECONDS), "now"),
        (NOW + dt.timedelta(seconds=ONE_MILLISECOND), "now"),
        (NOW + dt.timedelta(seconds=FOUR_MILLISECONDS), "now"),
        (NOW - dt.timedelta(seconds=2), "2 seconds ago"),
        (NOW - dt.timedelta(seconds=4), "4 seconds ago"),
        (NOW - dt.timedelta(seconds=ONE_HOUR + FOUR_MILLISECONDS), "1 hour and 0.00 seconds ago"),
        (NOW - dt.timedelta(seconds=ONE_DAY + FOUR_MILLISECONDS), "1 day and 0.00 seconds ago")
    ],
)
def test_precisetime_minimum_unit_default(arg, expected):
    # Act / Assert
    assert humanize.precisetime(arg) == expected
